 ...
 */
void write_mandel_line(int color_val[])
{
	int i;
...
void compute_and_write_mandel_line(char *buffer, int proc_n)
{
	/*
	...
	for (i = proc_n; i < y_chars; i += NPROCS) {
		...
		write_mandel_line(buffer, color_val);
	}
}
...
int main(int argc, char **argv)
{
	...
	for (proc_n = 0; proc_n < NPROCS; proc_n++) {
		...
		if (p == 0) {
			compute_and_write_mandel_line(1, proc_n);
			exit(1); // If child doesn't exit properly
		} 
	}
